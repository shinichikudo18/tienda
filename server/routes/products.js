...
router.post('/', requireRole('admin', 'supervisor'), (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO products (name, description, unit_type, base_quantity, base_cost, package_count, package_unit, selling_price, stock_quantity, min_stock, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      name,
      description || null,
  ...
  try {
    ...
    if (selling_price !== undefined && selling_price !== product.selling_price) {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?)
      `).run(req.params.id, product.selling_price, selling_price, req.user.id);
    }

    db.prepare(`
      UPDATE products SET
        name = ?, description = ?, unit_type = ?, base_quantity = ?, base_cost = ?,
        package_count = ?, package_unit = ?, selling_price = ?, stock_quantity = ?, min_stock = ?
      WHERE id = ?
    `).run(
      name || product.name,
...
router.post('/:id/assign-stock', requireRole('admin', 'supervisor'), (req, res) => {
  try {
    ...
    const user = db.prepare('SELECT * FROM users WHERE id = ? AND role = ?').get(user_id, 'vendedor');
    if (!user) {
      return res.status(400).json({ error: 'Usuario vendedor no encontrado' });
    }

    const existing = db.prepare('SELECT * FROM stock_assignments WHERE product_id = ? AND user_id = ?').get(req.params.id, user_id);
    
    if (existing) {
      db.prepare('UPDATE stock_assignments SET quantity_assigned = ? WHERE id = ?').run(quantity, existing.id);
    } else {
      db.prepare('INSERT INTO stock_assignments (product_id, user_id, quantity_assigned) VALUES (?, ?, ?)').run(req.params.id, user_id, quantity);
    }

    const assignment = db.prepare('SELECT * FROM stock_assignments WHERE product_id = ? AND user_id = ?').get(req.params.id, user_id);
    res.json(assignment);
  } catch (error) {
